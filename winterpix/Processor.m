...
- (UIImage *)addSnowfallWithImage:(UIImage *)image WithSnowfallImage:(UIImage *)snowImage
{
    ...
    for (int i = 0; i < numberOfRepeat; i++) {
        @autoreleasepool {
            ...
            [transform addTarget:blur];
            [blur addTarget:motion];
            motion.blurAngle = _snowDirection * -60.0f - 90.0f;
            motion.blurSize = 2.0 * absf(_snowDirection);
    ...
    for (int i = 0; i < numberOfRepeat; i++) {
        @autoreleasepool {
            ...
            GPUImageMotionBlurFilter* motion = [[GPUImageMotionBlurFilter alloc] init];
            GPUImageTransformFilter* transform = [[GPUImageTransformFilter alloc] init];
            transform.affineTransform = CGAffineTransformConcat(CGAffineTransformMakeScale(1.5, 1.5), CGAffineTransformMakeRotation(M_PI * (float)i));
            [transform addTarget:motion];
            motion.blurAngle = _snowDirection * -60.0f - 90.0f;
            motion.blurSize = 4.0 * absf(_snowDirection);
            [base addTarget:transform];
            [base processImage];
            image = [Processor mergeBaseImage:image overlayImage:[motion imageFromCurrentlyProcessedOutput] opacity:1.0f blendingMode:MergeBlendingModeScreen];
        }
    ...
    for (int i = 0; i < numberOfRepeat; i++) {
        @autoreleasepool {
            ...
            GPUImageMotionBlurFilter* motion = [[GPUImageMotionBlurFilter alloc] init];
            GPUImageTransformFilter* transform = [[GPUImageTransformFilter alloc] init];
            transform.affineTransform = CGAffineTransformConcat(CGAffineTransformMakeScale(2.0, 2.0), CGAffineTransformMakeRotation(M_PI * (float)i));
            [transform addTarget:motion];
            motion.blurAngle = _snowDirection * -60.0f - 90.0f;
            motion.blurSize = 6.0 * absf(_snowDirection);
            [base addTarget:transform];
            [base processImage];
            image = [Processor mergeBaseImage:image overlayImage:[motion imageFromCurrentlyProcessedOutput] opacity:1.0f blendingMode:MergeBlendingModeScreen];
        }
